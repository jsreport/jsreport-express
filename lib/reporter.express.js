...
module.exports = function (reporter, definition) {
  ...
  reporter.options.subPath = reporter.options.subPath || '/'
  ...
  if (reporter.options.subPath.substr(-1) !== '/') {
    reporter.options.subPath += '/'
  }
  ...
  reporter.initializeListeners.add(definition.name, this, async () => {
    ...
    configureExpressApp(app, reporter, definition)
    initialApp.use(reporter.options.subPath, app)
