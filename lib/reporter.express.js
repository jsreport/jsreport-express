...
var configureExpressApp = function (app, reporter, definition) {
  const baseApp = reporter.options.subPath ? express() : app

  reporter.express.app = baseApp

  if (definition.options.trustProxy !== false) {
    baseApp.enable('trust proxy')
  }

  baseApp.options('*', (req, res) => {
    require('cors')({
  ...
  baseApp.use(bodyParser.urlencoded({ extended: true, limit: definition.options.inputRequestLimit || '20mb' }))
  ...
  reporter.emit('before-express-configure', baseApp)

  routes(baseApp, reporter)

  reporter.emit('express-configure', baseApp)

  const handleErrorMiddleware = handleError(reporter)

  baseApp.use((err, req, res, next) => handleErrorMiddleware(req, res, err))

  if (reporter.options.subPath) {
    app.use(reporter.options.subPath, baseApp)
...
module.exports = function (reporter, definition) {
  ...
  reporter.initializeListeners.add(definition.name, this, async () => {
    ...
    const startFn = await prepareStartExpressApp(reporter, app, reporter.options, definition.options.start)
